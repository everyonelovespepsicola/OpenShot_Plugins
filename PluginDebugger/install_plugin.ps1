# Install Script for OpenShot PluginDebugger Plugin

$PluginName = "PluginDebugger"
...
if (Test-Path $ProgramFilesDir) {
    try {
        Copy-Item -Path (Join-Path $SourceDir "plugin_debugger.py") -Destination $ProgramFilesDir -Force -ErrorAction SilentlyContinue
