# Install Script for OpenShot ExportFFmpegCommand Plugin

$PluginName = "ExportFFmpegCommand"
...
if (Test-Path $ProgramFilesDir) {
    try {
        Copy-Item -Path (Join-Path $SourceDir "export_ffmpeg_command.py") -Destination $ProgramFilesDir -Force -ErrorAction SilentlyContinue
