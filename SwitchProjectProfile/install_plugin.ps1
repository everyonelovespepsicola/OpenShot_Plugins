# Install Script for OpenShot SwitchProjectProfile Plugin

$PluginName = "SwitchProjectProfile"
...
if (Test-Path $ProgramFilesDir) {
    try {
        Copy-Item -Path (Join-Path $SourceDir "switch_project_profile.py") -Destination $ProgramFilesDir -Force -ErrorAction SilentlyContinue
