# Install Script for OpenShot CleanUnusedMedia Plugin

$PluginName = "CleanUnusedMedia"
...
if (Test-Path $ProgramFilesDir) {
    try {
        Copy-Item -Path (Join-Path $SourceDir "clean_unused_media.py") -Destination $ProgramFilesDir -Force -ErrorAction SilentlyContinue
