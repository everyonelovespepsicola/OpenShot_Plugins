# Install Script for OpenShot PluginViewer Plugin

$PluginName = "PluginViewer"
...
if (Test-Path $ProgramFilesDir) {
    try {
        Copy-Item -Path (Join-Path $SourceDir "plugin_viewer.py") -Destination $ProgramFilesDir -Force -ErrorAction SilentlyContinue
